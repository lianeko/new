...
kj = LINETCR.LINE()
kj.login(token="ElgOCJ1ch0AAp1ABFMa7.DaiQwgTGNlaH0BREmT+F9W.hiUX+sHkLJAk5jrNhCWSKhTAs456fC1OO4by0f0b+Q8=")

kh = LINETCR.LINE()
kh.login(token="Elg80Oz8zCOQRcB3c5z8.l9c6r3yhq57co/nr9Dt3Ma.n6llLbZWrFsbemB1XE2LNcRMyezRkNBWy/lXd4q6/+c=")

ka = LINETCR.LINE()
ka.login(token="ElxPIQVeZhiGQY42JFKe.eEcj2QYedC7pblXT/RrFtG.hi9nrmVGjprtncrISn4fVJ4oWVqihmPop0S6X0M6s4s=")

kz = LINETCR.LINE()
kz.login(token="ElrRfZ49MxwsMboRjcIf.vw+0b/Qe/2oui1+jZd+53W.rRUfOLXs2SvqUNy75GRwCbukHIIGYPosIN9sb//I38I=")

ks = LINETCR.LINE()
ks.login(token="ElxjBK3X08RotOcFSvo3.VUHKteWikY+XShQkJ55KeW.xD2yUpM45ADbhOii7RAcAYS59ryEhyUWIlngufsZOIk=")

kb = LINETCR.LINE()
kb.login(token="El9MrqcexoJ2y1DdlJuf.aiqPzio9liPksFF/2uIRtW.6WqOqXGe3LHO8u8fQEyYlKWZ0/b4leUmwK8Ya7HTmq8=")

kw = LINETCR.LINE()
kw.login(token="ElPBN6G4qPO01QNgV4y1.vHfzTb2LLAHrUeCgOByFqq.qrrqsTRX6AToHPuWtwuZjWDH8TwXhauwKiaL4ZNKXRY=")
...
􀔃􀅕red arrow right􏿿 Command Kicker
...
✞[Block Invite -- Blockinvite On / Off]
✞[Protect Kick -- Protect Kick On / Off]
✞[Block Cancel -- Block Cancel On / Off]

✞My Creator Http://line.me/ti/p/~lianekof
"""
KAC=[cl,ki,kk,kc,kl,kj,kh,ka,kz,ks,kb,kw,"ubce1a713f0cd01fa3b402ebd3e72ecb1","u3b020c2365de7785208fd1ead0975575"]
...
Bots=[mid,Amid,Bmid,Cmid,Emid,Fmid,Gmid,Hmid,Imid,Jmid,Kmid,"ubce1a713f0cd01fa3b402ebd3e72ecb1"]
...
def bot(op):
    try:
        ...
        #------Fungsi Back UP start------#
                   
        #if op.type == 13:           
               #if op.param2 in Bots:
                   #cl.acceptGroupInvitation(op.param1,Bots)
                   #ki.acceptGroupInvitation(op.param1,Bots)
                   #kk.acceptGroupInvitation(op.param1,Bots)
                   #kc.acceptGroupInvitation(op.param1,Bots)
                   #kl.acceptGroupInvitation(op.param1,Bots)
                   #kj.acceptGroupInvitation(op.param1,Bots)
                   #kh.acceptGroupInvitation(op.param1,Bots)
                   #ka.acceptGroupInvitation(op.param1,Bots)
                   #kz.acceptGroupInvitation(op.param1,Bots)
                   #ks.acceptGroupInvitation(op.param1,Bots)
                   #kb.acceptGroupInvitation(op.param1,Bots)
                   #kw.acceptGroupInvitation(op.param1,Bots)
               #else:
                    #cl.acceptGroupInvitation(op.param1,Bots)
                    #ki.acceptGroupInvitation(op.param1,Bots)
                    #kk.acceptGroupInvitation(op.param1,Bots)
        ...
        #------Fungsi Back UP finish-----#

        #------Invite User Kick start------#
        if op.type == 13:
           if wait["Protectguest"] == True:
               if op.param2 not in Bots:
                  random.choice(KAC).cancelGroupInvitation(op.param1,Bots,[op.param3])
                ...
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:                            
                            ...
                            kz.rejectGroupInvitation(op.param1)
                            ks.rejectGroupInvitation(op.param1)
                            kb.rejectGroupInvitation(op.param1)
                            kw.rejectGroupInvitation(op.param1)
                        else:
			...
			if wait["atjointicket"] == True:
              ...
              if msg.from_ in admin:
               ...
               if msg.from_ in admin: 
                ...
                if wait["protection"] == True: md+=" Protect Kick : on\n"
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
               ...
               if msg.from_ in admin: 
                ...
                ki.sendText(msg.to,"Lord Angel 􀔃􀄆red check mark􏿿")
                kk.sendText(msg.to,"Wolf Police 􀔃􀄆red check mark􏿿")
                kc.sendText(msg.to,"Kaneki 􀔃􀄆red check mark􏿿")
                kl.sendText(msg.to,"Yatogami 􀔃􀄆red check mark􏿿")
                kj.sendText(msg.to,"Sawada Tsunayoshi 􀔃􀄆red check mark􏿿")
                kh.sendText(msg.to,"Kuroko 􀔃􀄆red check mark􏿿")
                ka.sendText(msg.to,"Seijuro 􀔃􀄆red check mark􏿿")
                kz.sendText(msg.to,"Aomine 􀔃􀄆red check mark􏿿")
                ks.sendText(msg.to,"Kirito 􀔃􀄆red check mark􏿿")
                kb.sendText(msg.to,"Yuuta 􀔃􀄆red check mark􏿿")
                kw.sendText(msg.to,"Kagami 􀔃􀄆red check mark􏿿")
      ...
      #-------------Fungsi Banned Send Contact Start------------------#
               ...
               if msg.from_ in admin: 
                ...
                kj.sendText(msg.to,"Kirim Kontak")
                kh.sendText(msg.to,"Kirim Kontak")
                ka.sendText(msg.to,"Kirim Kontak")
                kz.sendText(msg.to,"Kirim Kontak")
                ks.sendText(msg.to,"Kirim Kontak")
                kb.sendText(msg.to,"Kirim Kontak")
                kw.sendText(msg.to,"Kirim Kontak")
               ...
               if msg.from_ in admin: 
                if msg.toType == 2:
                    ...
                    if matched_list == []:
                        ...
                        kj.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        kh.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        ka.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        kz.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        ks.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        kb.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        kw.sendText(msg.to,"Disini Tidak Ada Blacklist")
                        return
                    for jj in matched_list:
                    ...
                    kj.sendText(msg.to,"Fuck You Blacklist")
                    kh.sendText(msg.to,"Fuck You Blacklist")
                    ka.sendText(msg.to,"Fuck You Blacklist")
                    kz.sendText(msg.to,"Fuck You Blacklist")
                    ks.sendText(msg.to,"Fuck You Blacklist")
                    kb.sendText(msg.to,"Fuck You Blacklist")
                    kw.sendText(msg.to,"Fuck You Blacklist")
